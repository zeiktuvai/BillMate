...
namespace Bill_Tracker
{
    ...
    private async void BillMate_Loaded(object sender, RoutedEventArgs e)
        {
            //TODO: Should create a DB using SQL instead of copying.
            await copyDB();
            ...
            {
                Debt.Visibility = ConfigurationManager.GetLocalSetting("EnableAcct").ToString() == "Visibility.Visible" ? Visibility.Visible : Visibility.Collapsed;
            }
            ...
            if (ConfigurationManager.CheckLocalSetting("AutoBkup") && ConfigurationManager.GetLocalSetting("AutoBkup").ToString() == "True")
            {
                ...
                if (bakupDate == null || DateTime.Today.Date > DateTime.Parse(bakupDate.ToString()).AddDays(30))
                {
                    ...
                    {
                        var result = await MicrosoftGraphProvider.backupDBODS();
                        if (result)
                        {
        ...
        private void lstMen_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            {
                navigatePage(typeof(acctLedger));
                tbxTitle.Text = "Ledger";
            }
            if (Debt.IsSelected)
            {
                navigatePage(typeof(Tracker));
                tbxTitle.Text = "Accounts";
            }
            if (History.IsSelected)
            {
                tbxTitle.Text = "Bill Management";
                navigatePage(typeof(payHistory));
            }
        ...
        private async Task copyDB()
        {
            try
            {
                await ApplicationData.Current.LocalFolder.GetFileAsync("bills.db3");
            }
        ...
        public void setAcctPageVis(bool vis)
        {
            Debt.Visibility = vis == true ? Visibility.Visible : Visibility.Collapsed;
        }
