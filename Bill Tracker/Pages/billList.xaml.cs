...
namespace Bill_Tracker
{
    ...
    public sealed partial class billList : Page
    {
        ...
        private void InitializeBillPage(int dateShift = 0)
        {
            ...
            //Sort bill list by date or category // TODO: add UI code for toggle switch.
        ...
        private string getSortOrder()
        {
            ...
            Object value = localsettings.Values["sortOrder"];
            if (value == null)
            {
                ConfigurationManager.SetLocalSetting("sortOrder", "strByDte");
                value = "strByDte";
            }
       ...
       //Pay clicked on menu flyout of bill item.
        ...
        {
            this.Frame.Navigate(typeof(payHistory), bill);
        }
        #endregion
        ...
        private void bttnQPay_Click(object sender, RoutedEventArgs e)
        {
            if (Global.Globals._WORKING_MONTH != -1)
            {
                ...
                if (Global.Globals._WORKING_MONTH == 1)
                {
                    qpay = new Payment() { amount = qpBill.baseAmount, BillID = qpBill.BillID, dispPayDate = DateTime.Now.AddMonths(1).Date.ToString("d") };
                }
                else
                {
                    qpay = new Payment() { amount = qpBill.baseAmount, BillID = qpBill.BillID, dispPayDate = DateTime.Now.Date.ToString("d") };
                }
