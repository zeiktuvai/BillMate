...
namespace Bill_Tracker
{
    ...
    /// </summary>
    public sealed partial class acctLedger : Page
    {
        //TODO: Show archived bills in the list
        ...
        private void ldgBttnSave_Click(object sender, RoutedEventArgs e)
        {
            ...
            else
            {
                if (tbxBillBdgt.Text == billBdgtAmnt.ToString())
                {
                    ...
                    //catch (Exception)
                    //{
                    //    BT.LocalIO.setSetting(billBdgtSettngName, tbxBillBdgt.Text);
                    //}         
                } else
                {                
                    ConfigurationManager.SetLocalSetting(billBdgtSettngName, tbxBillBdgt.Text);
        ...
        private void ldgPostAll_Click(object sender, RoutedEventArgs e)
        {
            ...
            //var notPost = list.Where(p => (p as fPayment).posted == false).ToList();
            //if (notPost.Count > 0)
            //{
            //    foreach (fPayment item in notPost)
            //    {
            //            item.posted = !item.posted;
            //            LocalIO.updatePaymen(Payment.parse(item));
            //    }
            //    ldgPostAll.InvalidateArrange();
            //} else
            //{
            //    foreach (fPayment item in list)
            //    {
            //        item.posted = !item.posted;
            //        LocalIO.updatePaymen(Payment.parse(item));
            //    }
            //}
            ////this.Frame.Navigate(typeof(acctLedger));
            //initializePage();
        }
